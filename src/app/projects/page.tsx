  ...
  return (
    <div className="min-h-screen bg-bg-light text-text-dark flex flex-col">
      ...
      <main className="flex-1 pt-28">
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-8">
            ...
            <div className="flex flex-wrap items-center justify-center gap-2 mb-12">
              {categories.map((cat) => (
                <button
                  ...
                  className={`px-5 py-2.5 rounded-none text-xs font-extrabold transition-all uppercase tracking-wider ${
                    activeFilter === cat.id
                      ? 'bg-gold-primary text-navy-dark shadow-md border border-navy-dark'
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
              {filteredProjects.map((proj) => (
                <div
                  key={proj.id}
                  className="bg-slate-50 border border-slate-200 rounded-none overflow-hidden shadow-lg hover:shadow-2xl transition-all duration-300 flex flex-col justify-between group"
                >
                  <div>
                    {/* Image Container */}
                    <div className="relative h-60 overflow-hidden cursor-pointer" onClick={() => setSelectedProject(proj)}>
                      <img
                        src={proj.image}
                        alt={proj.title}
                      ...
                      <div className="absolute top-4 left-4 bg-navy-primary text-gold-bright text-[10px] font-black uppercase px-3 py-1 shadow-md rounded-none border border-gold-primary/30">
                        {proj.sector}
                      </div>

                      {/* Expand Lightbox Icon */}
                      <div className="absolute bottom-4 right-4 p-2 bg-gold-primary text-navy-dark opacity-0 group-hover:opacity-100 transition-opacity shadow-md rounded-none">
                    ...
                    <div className="p-6 space-y-3">
                      ...
                      <div className="pt-3 border-t border-slate-200 space-y-1">
                        ...
                        <div className="flex flex-wrap gap-1">
                          {proj.scope.map((s, i) => (
                            <span key={i} className="text-[11px] font-semibold bg-white border border-slate-200 px-2 py-0.5 text-navy-primary rounded-none">
                              ✓ {s}
                            </span>
                          ))}
                        </div>
                  ...
                  <div className="p-6 pt-0">
                    <button
                      ...
                      className="w-full py-2.5 bg-navy-primary text-white hover:bg-gold-primary hover:text-navy-dark font-bold text-xs transition-colors rounded-none uppercase tracking-wider"
                    >
        ...
        {/* Lightbox Modal - Sharp Edge */}
        {selectedProject && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-navy-dark/90 backdrop-blur-md animate-fade-in">
            <div className="relative max-w-4xl w-full bg-white rounded-none overflow-hidden shadow-2xl border-2 border-gold-primary">
              <button
                onClick={() => setSelectedProject(null)}
                className="absolute top-4 right-4 z-10 p-2 bg-navy-dark text-white rounded-none hover:bg-gold-primary hover:text-navy-dark transition-colors border border-gold-primary"
              >
                <X className="w-6 h-6" />
              </button>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2">
                ...
                <div className="p-8 space-y-4 flex flex-col justify-between">
                  ...
                  <button
                    ...
                    className="w-full py-3.5 bg-gold-primary text-navy-dark font-extrabold text-xs uppercase rounded-none hover:bg-gold-bright transition-colors tracking-wider"
                  >
